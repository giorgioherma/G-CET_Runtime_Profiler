...
internal static class CompanionProfilerService
{
    ...
    public static CompanionCollectResult CollectLatest(AppSettings settings, string cetDestination)
    {
        ...
        {
            target = UniquePath(frameTimeRoot, Path.GetFileName(candidate.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)));
            count = CopyDirectoryVerified(candidate, target);
        }

        WriteManifest(cetDestination, status, sourceRoot, candidate, target, count, fingerprint, "Copied. External source was left untouched.");
    ...
    {
        if (File.Exists(path)) return Sha256(path);
        ...
        string note)
    {
        ...
        var manifest = new
        {
            ...
            kind = status.Kind,
            executable = status.ExeFound ? status.DisplayName : "",
